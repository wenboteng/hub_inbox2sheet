...
async function createLondonMarketIntelligenceReport() {
  ...
  try {
    ...
    const providerStats = londonActivities.reduce((acc, activity) => {
      ...
      if (activity.platform) acc[provider].platforms.add(activity.platform);
    ...
    const lowCompetitionCategories = Object.values(categoryStats)
      .filter((cat: any) => cat.count < 20 && cat.avgPrice > averagePrice)
...
| Price Range | £${Math.min(...data.priceGaps.map((g: any) => g.count > 0 ? g.count : 999999))} - £${Math.max(...data.priceGaps.map((g: any) => g.count > 0 ? g.count : 0))} | Market diversity |
...
${Object.entries(data.platformStats).map(([platform, stats]: [string, any]) => `
...
${data.topProviders.map((provider: any, index: number) => 
  `${index + 1} | ${provider.name} | ${provider.activityCount} | £${Math.round(provider.avgPrice * 100) / 100} | ${Math.round(provider.avgRating * 10) / 10}/5 | ${provider.platforms.join(', ')}`
).join('\n')}
...
${data.topCategories.map((cat: any, index: number) => 
  `${index + 1}. **${cat.name}**: ${cat.count} activities, £${Math.round(cat.avgPrice * 100) / 100} avg price, ${Math.round(cat.avgRating * 10) / 10}/5 avg rating`
).join('\n')}
...
2. **Platform Strategy**
   - Focus on ${Object.entries(data.platformStats).sort((a: any, b: any) => b[1].count - a[1].count)[0][0]} for maximum reach
