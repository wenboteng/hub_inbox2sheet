...
async function importMadridToSeparateTable() {
    ...
    try {
        ...
        const totalCount = await dual_prisma_1.gygPrisma.$queryRaw `SELECT COUNT(*) as count FROM madrid_activities`;
        ...
        const existingCount = await dual_prisma_1.mainPrisma.importedMadridActivity.count();
        if (existingCount > 0) {
            ...
            await dual_prisma_1.mainPrisma.importedMadridActivity.deleteMany();
        }
        // Process in batches
        const batchSize = 50;
                ...
                for (const activity of batch) {
                    try {
                        if (activity.id && activity.activity_name) {
                            ...
                            await dual_prisma_1.mainPrisma.importedMadridActivity.create({
                                data: {
...
`;
        ...
        await dual_prisma_1.mainPrisma.report.upsert({
            ...
            create: {
                ...
                slug: (0, slugify_1.slugify)('Madrid Activities Import Report Separate Table'),
                content: report,
                isPublic: false,
            },
            update: {
...
//# sourceMappingURL=import-madrid-to-separate-table.js.map