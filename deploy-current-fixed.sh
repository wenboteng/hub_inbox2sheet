...
# Install PostgreSQL if not already installed (non-interactive)
if ! command -v psql &> /dev/null; then
    echo "📥 Installing PostgreSQL..."
    # Pre-configure PostgreSQL to avoid interactive prompts
    echo "postgresql postgresql/version string 17" | debconf-set-selections
    echo "postgresql postgresql/start_on_boot boolean true" | debconf-set-selections
