...
exports.generateVendorAnalytics = generateVendorAnalytics;
...
async function generatePlatformStats(articles) {
...
function analyzeTopTopics(articles) {
    const topicKeywords = [
...
function analyzeLanguageDistribution(articles) {
...
function analyzeContentQuality(articles) {
...
function analyzeTrendingTopics(articles) {
    const thirtyDaysAgo = new Date();
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
    ...
    topicKeywords.forEach(topic => {
        const recentCount = recentArticles.filter(a => `${a.question} ${a.answer}`.toLowerCase().includes(topic)).length;
        const olderCount = olderArticles.filter(a => `${a.question} ${a.answer}`.toLowerCase().includes(topic)).length;
        if (recentCount > 0) {
...
async function generateVendorRecommendations(articles, insights) {
...
function createComprehensiveReport(insights, recommendations) {
...
//# sourceMappingURL=generate-vendor-analytics.js.map