...
exports.analyzeViennaInsightsPotential = analyzeViennaInsightsPotential;
    ...
    try {
        ...
        viennaActivities.forEach(activity => {
            ...
            if (name.includes('tour'))
                activityTypes.set('Tours', (activityTypes.get('Tours') || 0) + 1);
            if (name.includes('concert') || name.includes('music'))
                activityTypes.set('Concerts & Music', (activityTypes.get('Concerts & Music') || 0) + 1);
            if (name.includes('museum') || name.includes('gallery'))
                activityTypes.set('Museums & Galleries', (activityTypes.get('Museums & Galleries') || 0) + 1);
            if (name.includes('palace') || name.includes('castle'))
                activityTypes.set('Palaces & Castles', (activityTypes.get('Palaces & Castles') || 0) + 1);
            if (name.includes('food') || name.includes('wine') || name.includes('dining'))
                activityTypes.set('Food & Wine', (activityTypes.get('Food & Wine') || 0) + 1);
            if (name.includes('walking') || name.includes('hiking'))
                activityTypes.set('Walking & Hiking', (activityTypes.get('Walking & Hiking') || 0) + 1);
            if (name.includes('bike') || name.includes('cycling'))
                activityTypes.set('Biking', (activityTypes.get('Biking') || 0) + 1);
            if (name.includes('transport') || name.includes('transfer'))
                activityTypes.set('Transport', (activityTypes.get('Transport') || 0) + 1);
            // Analyze venues
            if (activity.venue) {
                const venue = activity.venue.toLowerCase();
                if (venue.includes('schönbrunn'))
                    venueTypes.set('Schönbrunn Palace', (venueTypes.get('Schönbrunn Palace') || 0) + 1);
                if (venue.includes('hofburg'))
                    venueTypes.set('Hofburg Palace', (venueTypes.get('Hofburg Palace') || 0) + 1);
                if (venue.includes('belvedere'))
                    venueTypes.set('Belvedere Palace', (venueTypes.get('Belvedere Palace') || 0) + 1);
                if (venue.includes('st. stephen') || venue.includes('stephens'))
                    venueTypes.set('St. Stephen\'s Cathedral', (venueTypes.get('St. Stephen\'s Cathedral') || 0) + 1);
                if (venue.includes('opera') || venue.includes('oper'))
                    venueTypes.set('Vienna State Opera', (venueTypes.get('Vienna State Opera') || 0) + 1);
            }
        });
        ...
        viennaActivities.forEach(activity => {
            if (activity.providerName) {
                ...
                if (activity.ratingNumeric)
                    providerData.avgRating += activity.ratingNumeric;
            ...
            if (activity.duration) {
                ...
                if (duration.includes('hour') || duration.includes('hr')) {
                    ...
                    if (hourMatch) {
                        ...
                        if (hours <= 2)
                            durationPatterns.set('Short (1-2 hours)', (durationPatterns.get('Short (1-2 hours)') || 0) + 1);
                        else if (hours <= 4)
                            durationPatterns.set('Medium (3-4 hours)', (durationPatterns.get('Medium (3-4 hours)') || 0) + 1);
                        else
                            durationPatterns.set('Long (5+ hours)', (durationPatterns.get('Long (5+ hours)') || 0) + 1);
                    }
                }
            }
        });
...
//# sourceMappingURL=analyze-vienna-insights-potential.js.map