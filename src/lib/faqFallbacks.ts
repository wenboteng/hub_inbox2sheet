...
export const FAQ_FALLBACKS: FAQFallback[] = [
  // Airbnb Payouts
  {
...
export function findMatchingFallback(query: string, platform?: string): FAQFallback | null {
  const normalizedQuery = query.toLowerCase();
  
  // Filter by platform if specified
  const relevantFallbacks = platform 
    ? FAQ_FALLBACKS.filter(fallback => 
        fallback.platform.toLowerCase() === platform.toLowerCase()
      )
    : FAQ_FALLBACKS;
  
  // Find the best match based on keyword overlap
  let bestMatch: FAQFallback | null = null;
  let bestScore = 0;
  
  for (const fallback of relevantFallbacks) {
    const matchingKeywords = fallback.triggerKeywords.filter(keyword =>
...
export function getFallbackForPlatform(platform: string, category?: string): FAQFallback[] {
  let fallbacks = FAQ_FALLBACKS.filter(fallback => 
    fallback.platform.toLowerCase() === platform.toLowerCase()
  );
  
  if (category) {
    fallbacks = fallbacks.filter(fallback => 
      fallback.category.toLowerCase() === category.toLowerCase()
    );
  }
