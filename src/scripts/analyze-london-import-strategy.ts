...
async function analyzeLondonImportStrategy() {
  ...
  try {
    ...
    const gygTotal = await gygPrisma.$queryRaw`SELECT COUNT(*) as count FROM gyg_activities`;
    const gygCount = Number((gygTotal as any[])[0]?.count || 0);
    ...
    const viatorTotal = await gygPrisma.$queryRaw`SELECT COUNT(*) as count FROM viator_activities`;
    const viatorCount = Number((viatorTotal as any[])[0]?.count || 0);
    ...
    const currentCleanedLondon = await mainPrisma.cleanedActivity.count({
      where: { city: 'London' }
    });
