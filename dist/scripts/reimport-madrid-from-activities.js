...
function parseTagsFromJSONB(tags) {
    if (!tags)
        return [];
...
async function reimportMadridFromActivities() {
    ...
    try {
        ...
        const madridActivities = await dual_prisma_1.gygPrisma.$queryRaw `
      SELECT * FROM activities WHERE location ILIKE '%madrid%'
    `;
        console.log(`📊 Found ${madridActivities.length} Madrid activities in GYG activities table`);
        let importedCount = 0;
        let skippedCount = 0;
        let errorCount = 0;
        for (const activity of madridActivities) {
            try {
                ...
                const exists = await dual_prisma_1.mainPrisma.importedGYGActivity.findUnique({
                    where: { originalId }
                });
                if (exists) {
                    skippedCount++;
...
`;
        await dual_prisma_1.mainPrisma.report.upsert({
            ...
            create: {
                ...
                slug: (0, slugify_1.slugify)('Madrid Activities Re-import Report'),
                content: report,
                isPublic: false,
            },
            update: {
...
//# sourceMappingURL=reimport-madrid-from-activities.js.map