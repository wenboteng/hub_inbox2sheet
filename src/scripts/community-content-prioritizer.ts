...
// Content priority scoring system
...
class CommunityContentPrioritizer {
   ...
   */
  private calculatePriorityScore(article: any): ContentPriorityScore {
    ...
    if (factors.isCommunity) score += 25;
    if (factors.isQuestionBased) score += 20;
    if (factors.hasProblemSolution) score += 15;
    if (factors.isTourVendorSpecific) score += 20;
    if (factors.isRecent) score += 10;
    if (factors.hasEngagement) score += 5;
    if (factors.isNotPromotional) score += 5;
    
    // Determine recommended action
    let recommendedAction: 'keep' | 'prioritize' | 'deprioritize' | 'remove';
    if (score >= 80) recommendedAction = 'prioritize';
    else if (score >= 60) recommendedAction = 'keep';
    else if (score >= 40) recommendedAction = 'deprioritize';
   ...
   */
  private isCommunityContent(article: any): boolean {
   ...
   */
  private isQuestionBased(article: any): boolean {
   ...
   */
  private hasProblemSolution(text: string): boolean {
   ...
   */
  private isTourVendorSpecific(text: string): boolean {
   ...
   */
  private isRecent(article: any): boolean {
   ...
   */
  private hasEngagement(article: any): boolean {
   ...
   */
  private isNotPromotional(text: string): boolean {
   ...
   */
  async prioritizeAllContent(): Promise<ContentPriorityScore[]> {
    ...
    const priorityScores: ContentPriorityScore[] = [];
   ...
   */
  async applyPrioritization(scores: ContentPriorityScore[]): Promise<void> {
