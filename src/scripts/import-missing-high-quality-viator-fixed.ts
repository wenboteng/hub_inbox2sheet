...
function truncateText(text: string | null, maxLength: number = 500): string | null {
  if (!text) return null;
...
async function importMissingHighQualityViatorFixed() {
  console.log('📥 IMPORTING MISSING HIGH-QUALITY VIATOR ACTIVITIES (FIXED)...\n');
  ...
  try {
    ...
    for (let i = 0; i < highQualityActivities.length; i++) {
      ...
      try {
        ...
        // Truncate description to avoid database errors
        const truncatedDescription = truncateText(activity.description, 500);
        
        // Import to cleaned table
...
importMissingHighQualityViatorFixed().catch(console.error); 