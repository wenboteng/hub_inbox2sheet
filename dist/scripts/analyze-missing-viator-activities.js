...
async function analyzeMissingViatorActivities() {
    ...
    `;
        ...
        missingActivities.forEach(activity => {
            if (activity.price)
                completenessStats.hasPrice++;
            if (activity.rating)
                completenessStats.hasRating++;
            if (activity.review_count)
                completenessStats.hasReviews++;
            if (activity.description)
                completenessStats.hasDescription++;
            if (activity.duration)
                completenessStats.hasDuration++;
            if (activity.activity_url)
                completenessStats.hasUrl++;
        });
...
//# sourceMappingURL=analyze-missing-viator-activities.js.map