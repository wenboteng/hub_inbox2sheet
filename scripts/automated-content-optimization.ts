...
async function automatedContentOptimization() {
  ...
  try {
    ...
    const platformAnalysis = new Map<string, { count: number; avgScore: number; improvements: string[] }>();
    ...
    writeFileSync(
      join(process.cwd(), 'content-optimization-report.json'),
...
function calculateSEOScore(analysis: SEOAnalysis): number {
  ...
  if (analysis.hasTargetKeywords) score += 25;
  
  // Internal links (15 points)
  if (analysis.hasInternalLinks) score += 15;
  
  // Schema markup (10 points)
  if (analysis.hasSchemaMarkup) score += 10;
  
  // Content length (15 points)
  if (analysis.contentLength >= 1000) score += 15;
  else if (analysis.contentLength >= 500) score += 10;
  else if (analysis.contentLength >= 200) score += 5;
  
  // Readability (10 points)
  if (analysis.readabilityScore >= 80) score += 10;
  else if (analysis.readabilityScore >= 60) score += 7;
  else if (analysis.readabilityScore >= 40) score += 4;
  
  // Keyword density (5 points)
  if (analysis.keywordDensity >= 0.5 && analysis.keywordDensity <= 3) score += 5;
