...
async function verifySecondDbLondonCounts() {
  ...
  try {
    ...
    const allGygActivities = await gygPrisma.$queryRaw`
      SELECT 
    ...
    const gygLondonActivities = (allGygActivities as any[]).filter(activity => 
    ...
    const gygInCleaned = await mainPrisma.cleanedActivity.count({
      where: { 
        city: 'London',
        platform: 'gyg'
      }
    ...
    const viatorInCleaned = await mainPrisma.cleanedActivity.count({
      where: { 
        city: 'London',
        platform: 'viator'
      }
    ...
    const gygWithQuality = gygLondonActivities.filter(activity => 
      activity.extraction_quality && activity.extraction_quality > 0
    );
    ...
    const viatorWithQuality = viatorLondonActivities.filter(activity => 
      activity.extraction_quality && activity.extraction_quality > 0
    );
