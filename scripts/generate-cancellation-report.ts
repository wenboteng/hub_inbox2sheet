...
interface CancellationData {
  ...
  seasonalPatterns: Array<{ period: string; frequency: number; reasons: string[] }>;
  costImpact: {
    averageRefundAmount: string;
    totalRefundVolume: string;
    percentageOfBookings: number;
  };
...
async function generateCancellationReport(): Promise<void> {
  ...
  try {
    ...
    const reportPath = join(process.cwd(), 'cancellation-reasons-report.md');
...
async function analyzeCancellationData(articles: any[]): Promise<CancellationData> {
...
function getKeywordsForReason(reason: string): string[] {
...
function createCancellationReport(data: CancellationData): string {
