...
async function monitorDataQuality() {
    ...
    try {
        ...
        // Get total count
        const totalActivities = await dual_prisma_1.mainPrisma.importedGYGActivity.count();
...
async function calculateQualityMetrics() {
    ...
    const priceCoverage = await dual_prisma_1.mainPrisma.importedGYGActivity.count({
        where: { priceNumeric: { not: null } }
    });
    const ratingCoverage = await dual_prisma_1.mainPrisma.importedGYGActivity.count({
        where: { ratingNumeric: { not: null } }
    });
    const locationCoverage = await dual_prisma_1.mainPrisma.importedGYGActivity.count({
        where: { location: { not: '' } }
    });
    const durationCoverage = await dual_prisma_1.mainPrisma.importedGYGActivity.count({
        where: {
            OR: [
    ...
    const tagsCoverage = await dual_prisma_1.mainPrisma.importedGYGActivity.count({
        where: { tags: { isEmpty: false } }
    });
    // Get quality score distribution
    const qualityScores = await dual_prisma_1.mainPrisma.importedGYGActivity.findMany({
    ...
    qualityScores.forEach(item => {
        ...
        if (score >= 90)
            qualityDistribution['90-100']++;
        else if (score >= 80)
            qualityDistribution['80-89']++;
        else if (score >= 70)
            qualityDistribution['70-79']++;
        else if (score >= 60)
            qualityDistribution['60-69']++;
        else if (score >= 50)
            qualityDistribution['50-59']++;
        else if (score >= 40)
            qualityDistribution['40-49']++;
        else if (score >= 30)
            qualityDistribution['30-39']++;
        else if (score >= 20)
            qualityDistribution['20-29']++;
        else if (score >= 10)
            qualityDistribution['10-19']++;
        else
...
async function saveQualityReport(report) {
    try {
...
//# sourceMappingURL=data-quality-monitor.js.map