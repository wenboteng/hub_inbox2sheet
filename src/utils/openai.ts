 ...
 */
export async function enrichReportTitleAndIntro(title: string, intro: string): Promise<{ enrichedTitle: string; enrichedIntro: string }> {
 ...
 */
export async function generateShareSuggestions(reportContent: string): Promise<string[]> {
...
TWEET_2: [second tweet]`;
  ...
  if (tweet1Match?.[1]) suggestions.push(tweet1Match[1].trim());
  if (tweet2Match?.[1]) suggestions.push(tweet2Match[1].trim());
 ...
 */
export async function enrichReportWithGPT(reportContent: string, originalTitle?: string): Promise<{
