import { testBothDatabases, mainPrisma, gygPrisma } from '../lib/dual-prisma';
...
async function testDualDatabase() {
  ...
  if (results.main.connected) {
    ...
    try {
      ...
      const articleCount = await mainPrisma.article.count();
  ...
  if (results.gyg.connected) {
    ...
    try {
      ...
      console.log(`✅ GYG database: Found ${(tables as any[]).length} tables`);
      console.log('   Tables:', (tables as any[]).map((t: any) => t.table_name).join(', '));
