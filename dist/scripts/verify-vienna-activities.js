...
exports.verifyViennaActivities = verifyViennaActivities;
...
async function verifyViennaActivities() {
    ...
    try {
        ...
        const allViennaMentioned = await dual_prisma_1.mainPrisma.importedGYGActivity.findMany({
            where: {
                OR: [
        ...
        for (const activity of uniqueViennaActivities) {
            ...
            if (hasProvider)
                qualityAnalysis.withProviderName++;
            if (hasRating)
                qualityAnalysis.withRating++;
            if (hasReviewCount)
                qualityAnalysis.withReviewCount++;
            if (hasPrice)
                qualityAnalysis.withPrice++;
...
${qualityAnalysis.missingData.slice(0, 20).map(activity => `- **${activity.name}** - Provider: ${activity.provider} | Rating: ${activity.rating} | Reviews: ${activity.reviews} | Price: ${activity.price}`).join('\n')}
...
//# sourceMappingURL=verify-vienna-activities.js.map