...
async function cleanupMadridDuplicates() {
  ...
  try {
    ...
    const totalGYG = await mainPrisma.importedGYGActivity.count();
    const totalMadrid = await mainPrisma.importedMadridActivity.count();
    ...
    const madridInMainTable = await mainPrisma.importedGYGActivity.findMany({
      where: {
        OR: [
    ...
    const deleteResult = await mainPrisma.importedGYGActivity.deleteMany({
      where: {
        id: { in: idsToDelete }
      }
    });
    ...
    // Check final counts
    const finalGYG = await mainPrisma.importedGYGActivity.count();
    const finalMadrid = await mainPrisma.importedMadridActivity.count();
...
`;
    ...
    await mainPrisma.report.upsert({
      ...
      create: {
        ...
        slug: slugify('Madrid Duplicates Cleanup Report'),
        content: report,
        isPublic: false,
      },
      update: {
