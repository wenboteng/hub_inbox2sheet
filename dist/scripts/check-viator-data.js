...
exports.checkViatorData = checkViatorData;
...
async function checkViatorData() {
    ...
    try {
        ...
        // Check total count
        const totalCount = await dual_prisma_1.gygPrisma.$queryRaw `SELECT COUNT(*) as count FROM viator_activities`;
        console.log(`📊 Total Viator activities in GYG database: ${totalCount[0].count}`);
        // Check sample data structure
        const sample = await dual_prisma_1.gygPrisma.$queryRaw `SELECT * FROM viator_activities LIMIT 3`;
        ...
        const missingFields = await dual_prisma_1.gygPrisma.$queryRaw `
      SELECT 
        COUNT(*) as total,
    ...
    `;
        ...
        const missingData = missingFields[0];
        ...
        // Check data quality
        const qualityCheck = await dual_prisma_1.gygPrisma.$queryRaw `
      SELECT 
        COUNT(*) as total,
    ...
    `;
        ...
        const qualityData = qualityCheck[0];
        ...
        const importableActivities = await dual_prisma_1.gygPrisma.$queryRaw `
    ...
    `;
        console.log(`\n✅ Activities that would pass import filter: ${importableActivities[0].count}`);
        // Check activities with location data
        const activitiesWithLocation = await dual_prisma_1.gygPrisma.$queryRaw `
      SELECT COUNT(*) as count 
      FROM viator_activities 
      WHERE location IS NOT NULL AND location != ''
    `;
        console.log(`📍 Activities with location data: ${activitiesWithLocation[0].count}`);
        // Check top locations
        const topLocations = await dual_prisma_1.gygPrisma.$queryRaw `
    ...
    `;
        ...
        // Check top providers
        const topProviders = await dual_prisma_1.gygPrisma.$queryRaw `
    ...
    `;
        ...
        activitiesWithoutLocation.forEach((row) => {
...
//# sourceMappingURL=check-viator-data.js.map