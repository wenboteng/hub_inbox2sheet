...
interface MarketInsights {
  ...
  averagePrice: number;
  averageRating: number;
  priceSegments: { budget: number; mid: number; premium: number };
  ratingDistribution: { excellent: number; good: number; average: number; poor: number };
  topVendors: VendorInsights[];
  marketOpportunities: string[];
  competitiveLandscape: any;
  activityTypeAnalysis: any;
}

async function analyzeMadridVendorInsightsClean(): Promise<void> {
  ...
  try {
    ...
    // Save report
    const reportPath = join(process.cwd(), 'madrid-vendor-insights-clean-report.md');
...
function categorizeMadridActivities(activities: any[]): any[] {
...
function categorizeMadridActivity(activityName: string): string {
...
function getActivityType(activityName: string): string {
  ...
  if (name.includes('walking')) return 'Walking Tour';
  if (name.includes('food') || name.includes('tapas')) return 'Food Tour';
  if (name.includes('day trip')) return 'Day Trip';
  if (name.includes('private')) return 'Private Tour';
  if (name.includes('bike')) return 'Bike Tour';
  if (name.includes('photo')) return 'Photo Tour';
  if (name.includes('night')) return 'Night Tour';
  if (name.includes('flamenco')) return 'Cultural Show';
  if (name.includes('stadium')) return 'Sports';
  return 'General Tour';
}

async function generateVendorInsights(data: any[]): Promise<VendorInsights[]> {
  ...
  const vendorInsights: VendorInsights[] = [];
...
async function generateMarketInsights(data: any[], vendorInsights: VendorInsights[]): Promise<MarketInsights> {
...
function analyzeActivityTypes(data: any[]): any {
  ...
  for (const [type, count] of Object.entries(typeCounts)) {
    const avgPrice = typePrices[type]?.length > 0 
      ? typePrices[type].reduce((a: number, b: number) => a + b, 0) / typePrices[type].length 
      : 0;
    const avgRating = typeRatings[type]?.length > 0 
      ? typeRatings[type].reduce((a: number, b: number) => a + b, 0) / typeRatings[type].length 
      : 0;
...
function identifyMarketOpportunities(data: any[], vendorInsights: VendorInsights[]): string[] {
...
function analyzeCompetitiveLandscape(vendorInsights: VendorInsights[]): any {
...
function generateComprehensiveReport(marketInsights: MarketInsights, vendorInsights: VendorInsights[]): string {
...
${Object.entries(marketInsights.activityTypeAnalysis).map(([type, data]: [string, any]) => `
### ${type}
