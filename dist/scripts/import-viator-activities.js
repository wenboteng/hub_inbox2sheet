...
exports.importViatorActivities = importViatorActivities;
...
function parseTagsFromJSONB(tags) {
    ...
    try {
        ...
        return Object.values(tags).filter(value => typeof value === 'string');
    }
    catch (error) {
        return [];
    }
...
async function importViatorActivities() {
    ...
    try {
        ...
        let viatorCount = 0;
        try {
            const countResult = await dual_prisma_1.gygPrisma.$queryRaw `SELECT COUNT(*) as count FROM viator_activities`;
            viatorCount = Number(countResult[0]?.count || 0);
...
//# sourceMappingURL=import-viator-activities.js.map