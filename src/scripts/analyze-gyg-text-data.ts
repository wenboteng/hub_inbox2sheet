...
async function analyzeGYGTextData() {
  ...
  try {
    ...
    const textPrices = await gygPrisma.$queryRaw`
    ...
    const textRatings = await gygPrisma.$queryRaw`
    ...
    const textReviews = await gygPrisma.$queryRaw`
    ...
    // Duration analysis
    const durations = await gygPrisma.$queryRaw`
    ...
    const activityNames = await gygPrisma.$queryRaw`
      SELECT 
        activity_name,
        provider_name,
        location,
...
${(qualityStats as any[]).map((q: any) => `- ${q.extraction_quality}: ${Number(q.count)} activities (${((Number(q.count) / totalRecords) * 100).toFixed(1)}%)`).join('\n')}
...
## Text-Based Price Analysis
${(textPrices as any[]).map((p: any) => `- ${p.price}: ${Number(p.count)} activities`).join('\n')}

## Text-Based Rating Analysis
${(textRatings as any[]).map((r: any) => `- ${r.rating}: ${Number(r.count)} activities`).join('\n')}

## Text-Based Review Count Analysis
${(textReviews as any[]).map((r: any) => `- ${r.review_count}: ${Number(r.count)} activities`).join('\n')}

## Duration Analysis
${(durations as any[]).map((d: any) => `- ${d.duration}: ${Number(d.count)} activities`).join('\n')}

## Sample High-Quality Activities
${(activityNames as any[]).map((activity: any) => 
  `- **${activity.activity_name}** (${activity.provider_name})
    - Location: ${activity.location}
    - Price: ${activity.price || 'N/A'}
    - Rating: ${activity.rating || 'N/A'}
    - Reviews: ${activity.review_count || 'N/A'}
    - Quality: ${activity.extraction_quality}`
).join('\n\n')}

## Data Insights
1. **High Quality Data**: ${((Number((qualityStats as any[]).find((q: any) => q.extraction_quality === 'High')?.count || 0) / totalRecords) * 100).toFixed(1)}% of activities have high extraction quality
