...
interface CustomerInsights {
  ...
  topQuestions: CustomerQuestion[];
  platformPreferences: Array<{ platform: string; questionCount: number; percentage: number }>;
  languageBreakdown: Array<{ language: string; questionCount: number; percentage: number }>;
  categoryAnalysis: Array<{ category: string; questionCount: number; avgLength: number }>;
}

async function generateCustomerInsights(): Promise<void> {
  ...
  try {
    ...
    const reportPath = join(process.cwd(), 'customer-insights-report.md');
...
async function analyzeCustomerBehavior(articles: any[]): Promise<CustomerInsights> {
...
function analyzePainPoints(articles: any[]): CustomerPainPoint[] {
  const painPointKeywords = {
    'cancellation': { sentiment: 'negative' as const, urgency: 'high' as const },
    'refund': { sentiment: 'negative' as const, urgency: 'high' as const },
    'problem': { sentiment: 'negative' as const, urgency: 'medium' as const },
    'issue': { sentiment: 'negative' as const, urgency: 'medium' as const },
    'error': { sentiment: 'negative' as const, urgency: 'high' as const },
    'failed': { sentiment: 'negative' as const, urgency: 'high' as const },
    'broken': { sentiment: 'negative' as const, urgency: 'high' as const },
    'not working': { sentiment: 'negative' as const, urgency: 'high' as const },
    'can\'t': { sentiment: 'negative' as const, urgency: 'medium' as const },
    'unable': { sentiment: 'negative' as const, urgency: 'medium' as const },
    'confused': { sentiment: 'neutral' as const, urgency: 'low' as const },
    'help': { sentiment: 'neutral' as const, urgency: 'medium' as const },
    'support': { sentiment: 'neutral' as const, urgency: 'medium' as const },
    'question': { sentiment: 'neutral' as const, urgency: 'low' as const },
    'how to': { sentiment: 'neutral' as const, urgency: 'low' as const }
  };

  const painPointCount = new Map<string, { count: number; platforms: Set<string>; sentiment: string; urgency: string }>();
  ...
  return Array.from(painPointCount.entries())
    .map(([topic, data]) => ({
      ...
      sentiment: data.sentiment as 'negative' | 'neutral' | 'positive',
...
function getTopQuestions(articles: any[]): CustomerQuestion[] {
  return articles
    .map(article => ({
...
function analyzePlatformPreferences(articles: any[]): Array<{ platform: string; questionCount: number; percentage: number }> {
...
function analyzeLanguageBreakdown(articles: any[]): Array<{ language: string; questionCount: number; percentage: number }> {
...
function analyzeCategories(articles: any[]): Array<{ category: string; questionCount: number; avgLength: number }> {
  const categoryData = new Map<string, { count: number; totalLength: number }>();
...
function createCustomerInsightsReport(insights: CustomerInsights): string {
...
export { generateCustomerInsights }; 