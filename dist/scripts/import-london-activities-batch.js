...
async function importLondonActivitiesBatch() {
    ...
    `;
        ...
        for (let offset = 0; offset < totalGyg; offset += batchSize) {
            ...
            const gygBatch = await dual_prisma_1.gygPrisma.$queryRaw `
        SELECT 
      ...
      `;
            const gygLondonBatch = gygBatch.filter(activity => isLondonActivity(activity.activity_name));
    ...
    `;
        ...
        for (let offset = 0; offset < totalViator; offset += batchSize) {
            ...
            const viatorBatch = await dual_prisma_1.gygPrisma.$queryRaw `
        SELECT 
          id, activity_name, provider_name, location, price, rating, review_count,
          duration, description, activity_url, extraction_quality
        ...
        LIMIT ${batchSize} OFFSET ${offset}
      `;
            const viatorLondonBatch = viatorBatch.filter(activity => isLondonActivity(activity.activity_name));
...
//# sourceMappingURL=import-london-activities-batch.js.map