...
async function debugImportDuplicates() {
    ...
    `;
        ...
        console.log('=============================================');
        const existingLondonActivities = await dual_prisma_1.mainPrisma.cleanedActivity.findMany({
            where: { city: 'London' },
            select: {
                id: true,
                activityName: true,
        ...
        console.log('=============================================');
        const sourceCounts = await dual_prisma_1.mainPrisma.cleanedActivity.groupBy({
            by: ['originalSource'],
            where: { city: 'London' },
            _count: { id: true }
        });
        ...
        const gygPlatformCount = await dual_prisma_1.mainPrisma.cleanedActivity.count({
            where: {
                city: 'London',
                platform: 'gyg'
            }
        ...
        const viatorPlatformCount = await dual_prisma_1.mainPrisma.cleanedActivity.count({
            where: {
                city: 'London',
                platform: 'viator'
            }
        ...
        if (sampleGygActivity) {
            ...
            const exactMatch = await dual_prisma_1.mainPrisma.cleanedActivity.findFirst({
                where: {
            ...
            const nameMatch = await dual_prisma_1.mainPrisma.cleanedActivity.findFirst({
                where: {
                    activityName: sampleGygActivity.activity_name,
                    city: 'London'
                }
...
//# sourceMappingURL=debug-import-duplicates.js.map