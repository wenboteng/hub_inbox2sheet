...
async function automatedReportSEOPipeline(): Promise<void> {
  ...
  try {
    ...
    const optimizedReports: NewReportSEO[] = [];
    ...
    const reportPath = join(process.cwd(), 'automated-report-seo-pipeline.md');
    ...
    for (const report of newReports) {
      ...
      const individualPath = join(process.cwd(), `new-report-seo-${report.type}.md`);
      writeFileSync(individualPath, individualReport, 'utf-8');
    }

    // Generate social media content calendar for new reports
    if (newReports.length > 0) {
      const socialMediaCalendar = createSocialMediaCalendar(newReports);
      const calendarPath = join(process.cwd(), 'new-reports-social-calendar.md');
      writeFileSync(calendarPath, socialMediaCalendar, 'utf-8');
    }
...
async function analyzeReportSEO(report: any): Promise<NewReportSEO> {
...
function extractKeywordsForReport(report: any): string[] {
...
function calculateSEOScore(wordCount: number, keywordCount: number, content: string): number {
  ...
  if (hasHeadings >= 10) score += 20;
  else if (hasHeadings >= 5) score += 15;
  else if (hasHeadings >= 3) score += 10;
  
  // Lists and formatting (max 20 points)
  const hasLists = (content.match(/^[-*+]\s+/gm) || []).length;
  if (hasLists >= 10) score += 20;
  else if (hasLists >= 5) score += 15;
  else if (hasLists >= 3) score += 10;
  
  return Math.min(score, 100);
}

function determinePriority(type: string, wordCount: number, seoScore: number): 'high' | 'medium' | 'low' {
  // High priority for new reports or low SEO scores
  if (seoScore < 50 || wordCount < 1000) return 'high';
...
function generateMetaDescription(report: any): string {
...
function identifyIndexingIssues(report: any): string[] {
  const issues = [];
...
function generateFixes(report: any): string[] {
  const fixes = [];
...
function createComprehensiveReport(reports: NewReportSEO[], newReports: NewReportSEO[]): string {
...
function createIndividualReport(report: NewReportSEO): string {
...
function createSocialMediaCalendar(newReports: NewReportSEO[]): string {
  if (newReports.length === 0) {
