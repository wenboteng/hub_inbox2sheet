...
async function checkCombinedGYGStats() {
    ...
    try {
        ...
        const gygCount = await dual_prisma_1.mainPrisma.importedGYGActivity.count();
        const madridCount = await dual_prisma_1.mainPrisma.importedMadridActivity.count();
        ...
        console.log('\n📈 MAIN GYG ACTIVITIES STATISTICS:');
        const gygStats = await dual_prisma_1.mainPrisma.importedGYGActivity.aggregate({
            _avg: {
                priceNumeric: true,
                ratingNumeric: true,
        ...
        const madridStats = await dual_prisma_1.mainPrisma.importedMadridActivity.aggregate({
            _avg: {
                priceNumeric: true,
                ratingNumeric: true,
                reviewCountNumeric: true,
        ...
        // Recent imports
        console.log('\n📅 RECENT IMPORTS:');
        const recentGYG = await dual_prisma_1.mainPrisma.importedGYGActivity.findMany({
        ...
        const recentMadrid = await dual_prisma_1.mainPrisma.importedMadridActivity.findMany({
            orderBy: { importedAt: 'desc' },
            take: 3,
            select: {
                activityName: true,
...
//# sourceMappingURL=check-combined-gyg-stats.js.map