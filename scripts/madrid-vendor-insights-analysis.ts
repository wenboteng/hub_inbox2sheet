...
async function analyzeMadridVendorInsights(): Promise<void> {
  ...
  try {
    ...
    const madridFromMain = await mainPrisma.importedGYGActivity.findMany({
      where: {
        location: {
          contains: 'Madrid'
        }
    ...
    const madridFromSeparate = await mainPrisma.importedMadridActivity.findMany();
    ...
    // Save report
    const reportPath = join(process.cwd(), 'madrid-vendor-insights-report.md');
...
async function combineMadridData(mainData: any[], separateData: any[]): Promise<any[]> {
  const combined = [];
