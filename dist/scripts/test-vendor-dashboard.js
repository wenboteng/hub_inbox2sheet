...
async function testVendorDashboard() {
    ...
    try {
        ...
        for (const city of testCities) {
            ...
            const activities = await dual_prisma_1.mainPrisma.importedGYGActivity.findMany({
                where: whereClause,
                select: {
                    id: true,
                    activityName: true,
        ...
        for (const category of categories) {
            const categoryActivities = await dual_prisma_1.mainPrisma.importedGYGActivity.findMany({
                where: {
                    tags: {
                        hasSome: [category]
                    }
...
//# sourceMappingURL=test-vendor-dashboard.js.map