...
async function testContentDeduplication() {
    ...
    const hash = (0, contentDeduplication_1.generateContentHash)(testContent);
    console.log(`✅ Content hash generated: ${hash.substring(0, 16)}...`);
    // Test 2: Check deduplication stats
    const stats = await (0, contentDeduplication_1.getDeduplicationStats)();
    ...
    const duplicateCheck = await (0, contentDeduplication_1.checkContentDuplicate)(hash);
    ...
    console.log(`   - Enabled: ${contentDeduplication_1.DEFAULT_DEDUP_CONFIG.enabled}`);
    console.log(`   - Algorithm: ${contentDeduplication_1.DEFAULT_DEDUP_CONFIG.hashAlgorithm}`);
    console.log(`   - Threshold: ${contentDeduplication_1.DEFAULT_DEDUP_CONFIG.similarityThreshold}`);
    console.log(`   - Min length: ${contentDeduplication_1.DEFAULT_DEDUP_CONFIG.minContentLength}`);
}
async function testGetYourGuidePagination() {
    ...
    try {
        ...
        const articles = await (0, getyourguide_1.crawlGetYourGuideArticlesWithPagination)(testUrls);
