...
async function analyzeContentForReports() {
  ...
  try {
    ...
    const keywordStats = new Map<string, { count: number, contexts: string[], platforms: Set<string> }>();
    ...
    for (const [topicName, keywords] of Object.entries(topicClusters)) {
      ...
      if (relevantArticles.length > 0) {
        ...
        let businessValue = '';
        if (topicName.includes('Booking')) businessValue = 'Direct revenue impact';
        else if (topicName.includes('Customer')) businessValue = 'Customer retention and satisfaction';
        else if (topicName.includes('Review')) businessValue = 'Online reputation and social proof';
        else if (topicName.includes('Pricing')) businessValue = 'Revenue optimization';
        else if (topicName.includes('Technology')) businessValue = 'Operational efficiency and competitive advantage';
        else businessValue = 'Operational improvement';
    ...
    for (const topic of topics.slice(0, 3)) {
      const relevantArticles = allArticles.filter(article => {
        const text = `${article.question} ${article.answer}`.toLowerCase();
        return topicClusters[topic.topic as keyof typeof topicClusters]?.some(keyword => text.includes(keyword));
      });
      ...
      for (const article of relevantArticles) {
        ...
        if (positiveMatches > negativeMatches && positiveMatches > neutralMatches) positiveCount++;
        else if (negativeMatches > positiveMatches && negativeMatches > neutralMatches) negativeCount++;
        else neutralCount++;
      }
    ...
    const platformAnalysis = new Map<string, { count: number, topics: string[], avgLength: number }>();
...
function generateReportTitle(topic: string): string {
...
function determineTargetAudience(topic: string): string {
