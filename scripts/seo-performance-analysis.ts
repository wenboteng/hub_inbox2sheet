...
async function analyzeSEOPerformance() {
  ...
  try {
    ...
    const platformAnalysis: ContentQuality[] = [];
    ...
    const seoReportPath = join(process.cwd(), 'seo-monitoring-report.json');
    
    if (existsSync(seoReportPath)) {
      try {
        const reportData = JSON.parse(readFileSync(seoReportPath, 'utf8'));
    ...
    const estimatedTraffic = Math.round(
      (totalArticles * 0.8) * // Assuming 80% of articles get some traffic
