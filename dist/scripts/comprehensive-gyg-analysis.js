...
exports.comprehensiveGYGAnalysis = comprehensiveGYGAnalysis;
...
async function comprehensiveGYGAnalysis() {
    ...
    try {
        ...
        // Get total count
        const totalActivities = await dual_prisma_1.gygPrisma.$queryRaw `SELECT COUNT(*) as count FROM activities`;
        const count = totalActivities[0].count;
        console.log(`📊 Total activities: ${count}`);
        // Analyze data quality distribution
        const qualityDistribution = await dual_prisma_1.gygPrisma.$queryRaw `
      SELECT 
        extraction_quality,
        COUNT(*) as count,
    ...
    `;
        ...
        const locationAnalysis = await dual_prisma_1.gygPrisma.$queryRaw `
      SELECT 
        location,
        COUNT(*) as count,
        ROUND(COUNT(*) * 100.0 / ${count}, 2) as percentage
    ...
    `;
        console.log('\n🌍 TOP LOCATIONS:');
        locationAnalysis.forEach((row) => {
            console.log(`  ${row.location}: ${row.count} activities (${row.percentage}%)`);
        });
        // Analyze provider data
        const providerAnalysis = await dual_prisma_1.gygPrisma.$queryRaw `
      SELECT 
        provider_name,
        COUNT(*) as count,
        ROUND(COUNT(*) * 100.0 / ${count}, 2) as percentage
    ...
    `;
        console.log('\n🏢 TOP PROVIDERS:');
        providerAnalysis.forEach((row) => {
            console.log(`  ${row.provider_name}: ${row.count} activities (${row.percentage}%)`);
        });
        // Analyze price data
        const priceAnalysis = await dual_prisma_1.gygPrisma.$queryRaw `
      SELECT 
        CASE 
    ...
    `;
        console.log('\n💰 PRICE DATA ANALYSIS:');
        priceAnalysis.forEach((row) => {
            console.log(`  ${row.price_type}: ${row.count} activities (${row.percentage}%)`);
        });
        // Analyze rating data
        const ratingAnalysis = await dual_prisma_1.gygPrisma.$queryRaw `
      SELECT 
        CASE 
    ...
    `;
        ...
        ratingAnalysis.forEach((row) => {
            console.log(`  ${row.rating_type}: ${row.count} activities (${row.percentage}%)`);
        });
        // Analyze duration data
        const durationAnalysis = await dual_prisma_1.gygPrisma.$queryRaw `
      SELECT 
        CASE 
    ...
    `;
        console.log('\n🏆 SAMPLE HIGH-QUALITY RECORDS:');
        highQualitySamples.forEach((record, index) => {
            console.log(`\n  Record ${index + 1}:`);
...
- **Data Quality**: Mixed (High: ${qualityDistribution.find((r) => r.extraction_quality === 'High')?.percentage || 0}%, Medium: ${qualityDistribution.find((r) => r.extraction_quality === 'Medium')?.percentage || 0}%)
- **Geographic Coverage**: ${locationAnalysis.length} unique locations
- **Provider Diversity**: ${providerAnalysis.length} unique providers

## Data Quality Assessment

### Current State
1. **Extraction Quality**: ${qualityDistribution.map((r) => `${r.extraction_quality || 'NULL'}: ${r.percentage}%`).join(', ')}
2. **Price Coverage**: ${priceAnalysis.find((r) => r.price_type === 'Euro Price')?.percentage || 0}% have Euro prices
3. **Rating Coverage**: ${ratingAnalysis.find((r) => r.rating_type === 'Numeric Rating')?.percentage || 0}% have numeric ratings
4. **Duration Coverage**: ${durationAnalysis.find((r) => r.duration_type !== 'No Duration')?.percentage || 0}% have duration information
...
`;
        ...
        await dual_prisma_1.gygPrisma.$queryRaw `
...
//# sourceMappingURL=comprehensive-gyg-analysis.js.map