...
async function checkRawReviewData() {
  ...
  try {
    ...
    const tableStructure = await gygPrisma.$queryRaw`
    ...
    const reviewColumns = (tableStructure as any[]).filter((col: any) => 
      col.column_name.toLowerCase().includes('review') || 
      col.column_name.toLowerCase().includes('rating') ||
      col.column_name.toLowerCase().includes('count')
    );

    console.log('\n🔍 Review-related columns found:');
    reviewColumns.forEach((col: any) => {
    ...
    const sampleRawData = await gygPrisma.$queryRaw`
      SELECT 
        id,
        activity_name,
        review_count,
    ...
    (sampleRawData as any[]).forEach((row: any, index: number) => {
    ...
    const reviewCountStats = await gygPrisma.$queryRaw`
      SELECT 
    ...
    const stats = (reviewCountStats as any[])[0];
    ...
    const viennaRawData = await gygPrisma.$queryRaw`
      SELECT 
    ...
    const viennaStats = (viennaRawData as any[])[0];
    ...
    const viennaWithReviews = await gygPrisma.$queryRaw`
      SELECT 
        id,
        activity_name,
        review_count,
    ...
    (viennaWithReviews as any[]).forEach((row: any, index: number) => {
      console.log(`\n${index + 1}. ${row.activity_name}`);
    ...
    const importedVienna = await mainPrisma.importedGYGActivity.findMany({
      where: {
        OR: [
...
${(tableStructure as any[]).map((col: any) => `- ${col.column_name}: ${col.data_type}`).join('\n')}

## 🔍 Review-Related Columns
${reviewColumns.map((col: any) => `- ${col.column_name}: ${col.data_type}`).join('\n')}
