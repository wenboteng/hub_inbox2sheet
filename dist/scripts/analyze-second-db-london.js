...
async function analyzeSecondDBLondon() {
    ...
    try {
        ...
        let gygTotal = 0;
        try {
            const gygCount = await dual_prisma_1.gygPrisma.$queryRaw `SELECT COUNT(*) as count FROM gyg_activities`;
            gygTotal = Number(gygCount[0]?.count || 0);
        ...
        let viatorTotal = 0;
        try {
            const viatorCount = await dual_prisma_1.gygPrisma.$queryRaw `SELECT COUNT(*) as count FROM viator_activities`;
            viatorTotal = Number(viatorCount[0]?.count || 0);
        ...
        try {
            const activitiesCount = await dual_prisma_1.gygPrisma.$queryRaw `SELECT COUNT(*) as count FROM activities`;
            activitiesTotal = Number(activitiesCount[0]?.count || 0);
        ...
        if (activitiesTotal > 0) {
          ...
          LIMIT 1000
        `;
                const genericLondonActivities = activities.filter(activity => isLondonActivity(activity.activity_name));
        ...
        for (const table of tables) {
            try {
                const structure = await dual_prisma_1.gygPrisma.$queryRaw `
...
//# sourceMappingURL=analyze-second-db-london.js.map