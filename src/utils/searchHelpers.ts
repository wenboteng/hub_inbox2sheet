 ...
 */
export function extractRelevantSnippet(content: string, query: string, maxLength: number = 300): string {
  ...
  const scoredParagraphs = paragraphs.map(para => {
    ...
    terms.forEach(term => {
      ...
      if (count > 0) {
        ...
        if (pos < para.length / 3) score *= 1.3;
      }
    });
 ...
 */
export function rankArticlesByRelevance(articles: any[], query: string): any[] {
  const terms = query.toLowerCase().split(/\s+/).filter(term => term.length > 2);
