...
async function showContentExamples() {
  ...
  try {
    ...
    const highQualityArticles = articles.filter(a => 
      a.question && a.question.trim().length > 0 &&
      a.answer.length >= 100 && a.answer.length <= 2000 &&
      /tour|guide|booking|reservation|package|excursion|activity|attraction|hotel|accommodation|travel|trip|destination|cruise|flight|airline|vacation|holiday/.test((a.question + ' ' + a.answer).toLowerCase())
    ).slice(0, 5);
