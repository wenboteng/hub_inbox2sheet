...
async function generateSEOStrategy() {
  ...
  try {
    ...
    const contentAnalysis = new Map<string, Set<string>>();
    articles.forEach(article => {
      ...
      Object.keys(keywordCategories).forEach(category => {
        ...
        keywordCategories[category as keyof typeof keywordCategories].forEach(keyword => {
    ...
    const highPriorityKeywords = keywordOpportunities
      ...
      .sort((a, b) => {
        if (a.searchVolume === 'high' && b.searchVolume !== 'high') return -1;
        if (b.searchVolume === 'high' && a.searchVolume !== 'high') return 1;
