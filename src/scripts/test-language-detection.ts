import { detectLanguage, getLanguageName, getLanguageFlag } from '../utils/languageDetection';
...
async function testLanguageDetection() {
  ...
  for (const sample of testSamples) {
    const result = detectLanguage(sample.text);
    ...
    console.log(`   Detected: ${result.language} (${getLanguageName(result.language)}) ${getLanguageFlag(result.language)}`);
    console.log(`   Expected: ${sample.expected} (${getLanguageName(sample.expected)}) ${getLanguageFlag(sample.expected)}`);
