...
exports.EnhancedTourVendorCollector = void 0;
...
class EnhancedTourVendorCollector {
     ...
     */
    async processEnhancedContent(content) {
        try {
            ...
            if (isCommunity)
                this.stats.communityArticles++;
            if (isOTA)
                this.stats.otaArticles++;
            // Save to database
     ...
     */
    async collectEnhancedTourVendorContent() {
        ...
        for (const query of ENHANCED_TOUR_VENDOR_QUERIES) {
            ...
            try {
                ...
                const googleResults = await oxylabs_1.oxylabsScraper.searchContent(query, 'google', 8);
                console.log(`   Google results: ${googleResults.length}`);
                // Search on Bing with community focus
                const bingResults = await oxylabs_1.oxylabsScraper.searchContent(query, 'bing', 8);
     ...
     */
    async collectFromCommunityPlatforms() {
     ...
     */
    async collectFromOTAHelpCenters() {
...
exports.EnhancedTourVendorCollector = EnhancedTourVendorCollector;
// Main execution
async function main() {
...
//# sourceMappingURL=enhanced-tour-vendor-collector.js.map