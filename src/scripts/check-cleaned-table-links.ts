...
async function checkCleanedTableLinks() {
  ...
  try {
    ...
    const cleanedStructure = await mainPrisma.$queryRaw`
    ...
    (cleanedStructure as any[]).forEach(column => {
    ...
    const urlFieldExists = (cleanedStructure as any[]).some(col => col.column_name === 'url');
    ...
    if (urlFieldExists) {
      ...
      const activitiesWithUrl = await mainPrisma.cleanedActivity.count({
        where: { url: { not: null } }
      });
      ...
      const sampleWithUrls = await mainPrisma.cleanedActivity.findMany({
        where: { url: { not: null } },
        select: {
          id: true,
          activityName: true,
    ...
    const londonTotal = await mainPrisma.cleanedActivity.count({
      where: { city: 'London' }
    });
    
    const londonWithUrl = await mainPrisma.cleanedActivity.count({
      where: { 
        city: 'London',
        url: { not: null }
      }
    ...
    const gygStructure = await gygPrisma.$queryRaw`
      SELECT column_name, data_type, is_nullable
      FROM information_schema.columns 
    ...
    const gygUrlField = (gygStructure as any[]).find(col => 
      col.column_name === 'url' || 
    ...
    const viatorStructure = await gygPrisma.$queryRaw`
      SELECT column_name, data_type, is_nullable
      FROM information_schema.columns 
    ...
    const viatorUrlField = (viatorStructure as any[]).find(col => 
      col.column_name === 'url' || 
      col.column_name === 'activity_url' || 
      col.column_name === 'link'
    );
    ...
    try {
      const gygSample = await gygPrisma.$queryRaw`
      ...
      (gygSample as any[]).forEach((activity, index) => {
    ...
    try {
      const viatorSample = await gygPrisma.$queryRaw`
        SELECT activity_name, activity_url, url
      ...
      (viatorSample as any[]).forEach((activity, index) => {
        console.log(`${index + 1}. "${activity.activity_name.substring(0, 50)}..."`);
        console.log(`   activity_url: ${activity.activity_url || 'N/A'}`);
        console.log(`   url: ${activity.url || 'N/A'}`);
        console.log('');
